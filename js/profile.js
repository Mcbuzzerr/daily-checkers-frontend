...
const handleSaveClicked = (event) => {
    ...
    const url = `https://hjpe29d12e.execute-api.us-east-1.amazonaws.com/${STAGE}/user/update/${user.id}`;
...
const handleDeleteClicked = (event) => {
    ...
    const user = getUser();
    const url = 'https://hjpe29d12e.execute-api.us-east-1.amazonaws.com/${STAGE}/user/delete/' + user.id;
    const token = getCookie('authToken');
    fetch(url, {
        method: 'DELETE',
        headers: {
...
const handleSaveCustomizeClicked = (event) => {
    ...
    const url = "https://hjpe29d12e.execute-api.us-east-1.amazonaws.com/${STAGE}/user/update-customization/" + id;
    const token = getCookie('authToken');
    fetch(url, {
        method: 'PUT',
        headers: {
...
window.onload = () => {
    ...
    if (user_id == null) {
        ...
        const url = `https://hjpe29d12e.execute-api.us-east-1.amazonaws.com/${STAGE}/user/view/${id}`;
        fetch(url, {
            headers: {
                'content-type': 'application/json',
        ...
        const url = `https://hjpe29d12e.execute-api.us-east-1.amazonaws.com/${STAGE}/user/view/${user_id}`;
        fetch(url)
            .then(
                response => {
                    if (response.status === 404) {
