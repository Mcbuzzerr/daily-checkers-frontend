...
const getInviteList = async () => {
    let inviteList = [];
    let url = `https://hjpe29d12e.execute-api.us-east-1.amazonaws.com/${STAGE}/invite/list`
...
const declineInviteClicked = async (inviteId) => {
    ...
    buttons[1].style.transform = "translate(4px, 4px)";
    buttons[1].style.boxShadow = "none";
    fetch("https://hjpe29d12e.execute-api.us-east-1.amazonaws.com/${STAGE}/invite/decline/" + inviteId, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${getCookie('authToken')}`,
...
const handleNewInviteClicked = async () => {
    ...
    if (friendCode) {
        let url = `https://hjpe29d12e.execute-api.us-east-1.amazonaws.com/${STAGE}/invite/${friendCode}`;
        fetch(url, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
